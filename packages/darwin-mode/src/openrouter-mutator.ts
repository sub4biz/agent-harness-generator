...
export class OpenRouterMutator implements CodeGenerator {
  ...
  async generateMutation(input: {
    ...
    try {
      ...
      return { code: input.parentCode, summary: `openrouter:${this.model} unreachable (${(e as Error).message}) — no-op` };
    }
